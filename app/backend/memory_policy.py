"""Persistent opt-in local music model memory policy."""
...
def _release(reason: str) -> dict:
    ...
    with _LOCK:
        ...
        if _MANAGER is None:
            raise HTTPException(503, "The music generation manager is not ready")
        if _MANAGER.has_active_jobs():
            raise HTTPException(409, "Music generation is queued or running; memory was not released")
        _RELEASING = True
    try:
...
def run_due_release(now: float | None = None) -> dict | None:
    ...
    with _LOCK:
        ...
        if threshold is None or manager is None or _RELEASING:
            return None
        if manager.has_active_jobs() or not manager.has_loaded_model():
            return None
...
def status() -> dict:
    with _LOCK:
        ...
        threshold = MODES[mode]["idle_seconds"]
        manager = _MANAGER
        loaded_key = manager.loaded_model_key() if manager else None
        ...
        active = manager.has_active_jobs() if manager else False
        return {
            "mode": mode,
            "default_mode": DEFAULT_MODE,
            "idle_seconds": threshold,
            "options": [{"mode": key, **value} for key, value in MODES.items()],
            "loaded_model": list(loaded_key) if loaded_key else None,
            "model_idle_seconds": idle,
            "active_jobs": bool(active),

"""Automatic retention and hard-cap cleanup for completed music outputs."""
...
def _snapshot(output_dir: Path) -> dict:
    ...
    return {
        "used_bytes": sum(row[1] for row in rows), "count": len(rows),
...
def status(manager, output_dir: Path) -> dict:
    ...
    return {
        **policy, **{key: value for key, value in snap.items() if key != "rows"},
        "max_bytes": maximum,
        "over_limit": policy["enabled"] and snap["used_bytes"] > maximum,
        "scope": "generated music outputs only",
    }


def _remove(manager, path: Path) -> int:
    job = manager.get(path.stem)
    if job is not None and str(getattr(job, "state", "")) not in {"done", "error", "cancelled"}:
        return 0
    try:
...
def enforce(manager, output_dir: Path, target_bytes: int | None = None) -> dict:
    with _LOCK:
        ...
        result.update(used_bytes=final["used_bytes"], count=final["count"],

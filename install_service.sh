#!/bin/bash
#
# Install Music Studio KH as an always-on macOS service (launchd LaunchAgent).
#
...
SRV="com.kh.musicstudio.server"
WD="com.kh.musicstudio.watchdog"
PORT=47869
APPNAME="Music Studio KH"

mkdir -p "$LA" "$ROOT/logs/service" "$ROOT/service"
chmod +x "$ROOT/musicstudio-serve.sh" "$ROOT/musicstudio-watchdog.sh"
...
<dict>
  ...
  <array><string>$ROOT/musicstudio-serve.sh</string></array>
...
<dict>
  ...
  <key>ProgramArguments</key>
  <array><string>$ROOT/musicstudio-watchdog.sh</string></array>
  <key>RunAtLoad</key><true/>
